...
class PipelineRW:
    ...
    def calculate_metrics(self,
        ...
        directory = f"{self.data_home}/results/metrics/{self.run_name}/"
    ...
    def plotting(self, df_metrics, df_optimal_uplift, split="Train"):
        ...
        UpliftEvaluation.plot_qini_curve(df_metrics=df_metrics, df_opt_uplift_bins=df_optimal_uplift, plot_optimum=self.plot_optimum, fontsize=self.fontsize,
                                         ...
                                         save_figure=self.plot_save_figures, path=self.data_home, plot_grayscale=self.plot_grayscale, plot_uqc=self.plot_uqc, bins=self.bins,
    ...
    def calculate_feature_importance_mean(self, feature_importances, feature_names):
        ...
        if isinstance(feature_importances, list):
            ...
            for key, value in feature_importances_sum_dict.items():
                ...
                HelperPipeline.save_feature_importance(importance, feature_names, self.run_name + "_Feature_importance_{}".format(key), self.plot_save_figures, self.plot_figures)
        else:
