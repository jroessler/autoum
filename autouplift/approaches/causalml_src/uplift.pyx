...
class UpliftTreeClassifier:
    ...
    honesty: bool (default=False)
         True if the honest approach based on "Athey, S., & Imbens, G. (2016). Recursive partitioning for heterogeneous causal effects." shall be used.
    ...
    def __init__(self, control_name, max_features=None, max_depth=3, min_samples_leaf=100,
                 ...
                 normalization=True, honesty=False, random_state=None):
    ...
    def fit(self, X, treatment, y):
        ...
        if self.honesty:
            try:
                X, X_est, treatment_idx, treatment_idx_est, y, y_est = sklearn.model_selection.train_test_split(X, treatment_idx, y, stratify=[treatment_idx, y], test_size=0.5,
                                                                                                                shuffle=True, random_state=self.random_state)
            except ValueError:
                X, X_est, treatment_idx, treatment_idx_est, y, y_est = sklearn.model_selection.train_test_split(X, treatment_idx, y, test_size=0.5, shuffle=True,
